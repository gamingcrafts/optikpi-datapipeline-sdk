...
FormData$2.prototype._generateBoundary = function () {
  ...
  this._boundary = '--------------------------' + crypto$3.randomBytes(12).toString('hex');
};
 ...
 */
function deriveKey$1(authToken, accountId, workspaceId, info, algorithm = 'sha256', length = 32) {
  ...
  try {
    const derivedKey = crypto$1.hkdfSync(algorithm, saltBuffer, ikmBuffer, infoBuffer, length);
    return derivedKey;
  } catch (error) {
 ...
 */
function generateHmacSignature$2(data, authToken, accountId, workspaceId, algorithm = 'sha256') {
  ...
  try {
    ...
    const derivedKey = deriveKey$1(authToken, accountId, workspaceId, info, algorithm);
    const hmac = crypto$1.createHmac(algorithm, derivedKey);
 ...
 */
function validateHmacSignature$1(data, signature, authToken, accountId, workspaceId, algorithm = 'sha256') {
  try {
    const expectedSignature = generateHmacSignature$2(data, authToken, accountId, workspaceId, algorithm);
...
var crypto_1 = {
  ...
  generateHmacSignature: generateHmacSignature$2,
  validateHmacSignature: validateHmacSignature$1
};

const _excluded = ["authToken"];
const axios = axios_1;
const {
  generateHmacSignature: generateHmacSignature$1
} = crypto_1;
...
let DataPipelineClient$1 = class DataPipelineClient {
  ...
  setupAxiosInstance() {
    ...
    this.axios.interceptors.request.use(config => {
      if (config.data && config.method !== 'get') {
        const hmacSignature = generateHmacSignature$1(config.data, this.config.authToken, this.config.accountId, this.config.workspaceId);
  ...
  async sendBatch(batchData) {
    ...
    if (batchData.customers) {
      promises.push(this.sendCustomerProfile(batchData.customers).then(result => {
        results.customers = result;
      }));
    }
    if (batchData.accountEvents) {
      promises.push(this.sendAccountEvent(batchData.accountEvents).then(result => {
        results.accountEvents = result;
      }));
    }
    if (batchData.depositEvents) {
      promises.push(this.sendDepositEvent(batchData.depositEvents).then(result => {
        results.depositEvents = result;
      }));
    }
    if (batchData.withdrawEvents) {
      promises.push(this.sendWithdrawEvent(batchData.withdrawEvents).then(result => {
        results.withdrawEvents = result;
      }));
    }
  ...
  getConfig() {
    const _this$config = this.config,
      ...
      safeConfig = _objectWithoutProperties(_this$config, _excluded);
    return _objectSpread2(_objectSpread2({}, safeConfig), {}, {
      authToken: authToken ? `${authToken.substring(0, 8)}...` : undefined
    });
  }
};
var DataPipelineClient_1$1 = DataPipelineClient$1;

/**
 * Customer Profile Model
 * Represents a customer profile for the Data Pipeline API
 */
let CustomerProfile$2 = class CustomerProfile {
  constructor(data = {}) {
...
var CustomerProfile_1$1 = CustomerProfile$2;

/**
 * Account Event Model
 * Represents account-related events for the Data Pipeline API
 */
let AccountEvent$2 = class AccountEvent {
  constructor(data = {}) {
    this.account_id = data.account_id;
    this.workspace_id = data.workspace_id;
    this.user_id = data.user_id;
...
var AccountEvent_1$1 = AccountEvent$2;

/**
 * Deposit Event Model
 * Represents deposit-related events for the Data Pipeline API
 */
let DepositEvent$2 = class DepositEvent {
  constructor(data = {}) {
    this.account_id = data.account_id;
    this.workspace_id = data.workspace_id;
    this.user_id = data.user_id;
  ...
  validate() {
    ...
    // Event category validation
    if (this.event_category && this.event_category !== 'Deposit') {
  ...
  /**
   * Creates a DepositEvent instance from plain object
   * @param {Object} data - Plain object data
   * @returns {DepositEvent} New DepositEvent instance
   */
  static fromObject(data) {
    return new DepositEvent(data);
  }
};
var DepositEvent_1$1 = DepositEvent$2;

/**
 * Withdraw Event Model
 * Represents withdrawal-related events for the Data Pipeline API
 */
let WithdrawEvent$2 = class WithdrawEvent {
  constructor(data = {}) {
    this.account_id = data.account_id;
    this.workspace_id = data.workspace_id;
    this.user_id = data.user_id;
...
var WithdrawEvent_1$1 = WithdrawEvent$2;

/**
 * Gaming Activity Event Model
 * Represents gaming activity events for the Data Pipeline API
 */
let GamingActivityEvent$2 = class GamingActivityEvent {
  constructor(data = {}) {
    this.account_id = data.account_id;
    this.workspace_id = data.workspace_id;
    this.user_id = data.user_id;
 ...
 * @license MIT
 */
const DataPipelineClient = DataPipelineClient_1$1;
const {
  CustomerProfile,
  AccountEvent,
  DepositEvent,
...
src$1.exports = OptikpiDataPipelineSDK;

// Export individual components for advanced usage
var DataPipelineClient_1 = src$1.exports.DataPipelineClient = DataPipelineClient;
var CustomerProfile_1 = src$1.exports.CustomerProfile = CustomerProfile;
var AccountEvent_1 = src$1.exports.AccountEvent = AccountEvent;
...
var createClient = src$1.exports.createClient = config => new OptikpiDataPipelineSDK(config);
var srcExports = src$1.exports;
var index = /*@__PURE__*/getDefaultExportFromCjs(srcExports);

export { AccountEvent_1 as AccountEvent, CustomerExtEvent_1 as CustomerExtEvent, CustomerProfile_1 as CustomerProfile, DataPipelineClient_1 as DataPipelineClient, DepositEvent_1 as DepositEvent, GamingActivityEvent_1 as GamingActivityEvent, ReferFriendEvent_1 as ReferFriendEvent, SystemEvent_1 as SystemEvent, WalletBalanceEvent_1 as WalletBalanceEvent, WithdrawEvent_1 as WithdrawEvent, createClient, crypto, index as default };
